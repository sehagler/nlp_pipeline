...
class Remote_registry(object):
    ...
    def __init__(self, static_data_object, update_static_data_object,
                 directory_object, logger_object, root_dir, password):
        self.remote_manager_registry = {}
        self._server_manager(static_data_object, directory_object,
                             logger_object, password)
        self._update_manager(static_data_object, update_static_data_object,
                             directory_object, logger_object, root_dir,
                             password)
            
    #
    def _server_manager(self, static_data_object, directory_object,
                        logger_object, password):
        self.remote_manager_registry['server_manager'] = \
            Server_manager(static_data_object, directory_object,
                           logger_object, password)
    ...
    def _update_manager(self, static_data_object, update_static_data_object,
        ...
        self.remote_manager_registry['update_manager'] = \
            Server_manager(update_static_data_object, directory_object,
                           logger_object, password)
