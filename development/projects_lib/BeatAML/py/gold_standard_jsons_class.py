...
class Gold_standard_jsons(Packaging_base):
    ...
    def __init__(self, directory_manager, mrn_list):
        self.deidentifier_xlsx = directory_manager.pull_directory('raw_data_dir') + '/manuscript OHSU MRNs.xlsx'
        self.gpld_standard_xlsx = directory_manager.pull_directory('raw_data_dir') + '/Sup Table 5 Clinical summary.xlsx'
    ...
    def _get_data_dict(self, patientIds_list, labIds_list):
        ...
        patientIds = sheet.col_values(1)[1:]
        antibodies_tested = sheet.col_values(159)[1:]
        bone_marrow_blasts = self._make_strings(sheet.col_values(61)[1:])
        diagnoses = self._make_strings(sheet.col_values(28)[1:])
        diagnosis_date = sheet.col_values(160)[1:]
        fab_morphology = sheet.col_values(161)[1:]
        fish_analysis_summary = relapse_date = sheet.col_values(163)[1:]
        karyotype = sheet.col_values(76)[1:]
        peripheral_blood_blasts = self._make_strings(sheet.col_values(62)[1:])
        relapse_date = sheet.col_values(162)[1:]
        residual_diagnosis = sheet.col_values(164)[1:]
        specific_diagnoses = self._make_strings(sheet.col_values(29)[1:])
        surface_antigens = sheet.col_values(81)[1:]
