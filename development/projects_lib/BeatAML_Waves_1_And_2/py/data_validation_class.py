...
from projects_lib.BeatAML_Waves_1_And_2.py.beataml_waves_1_and_2_packager_class \
    import BeatAML_Waves_1_And_2_packager
from projects_lib.BeatAML_Waves_1_And_2.py.gold_standard_jsons_class import Gold_standard_jsons
from projects_lib.BeatAML_Waves_1_And_2.py.project_lib.packaging_base_class import Packaging_base

#
class Data_validation(Packaging_base):
    
    #
    def __init__(self, mrn_list, directory_manager):
        self.mrn_list = mrn_list
        self.directory_manager = directory_manager
        self.data_dir = self.directory_manager.pull_directory('postprocessing_data_out')
        ...
        patientIds.extend(self.validation_data.keys())
            ...
            for labId in labIds:
                ...
                for item in [ 'Antibodies.Tested', 'dx.Date', 'dx', 'Extramedullary.dx',
                    ...
                    if item in [ '%.Blasts.in.BM', '%.Blasts.in.PB' ]:
                        ...
                        item_score = \
                            self._compare_values(validation_record, gold_standard_record, item, item_score, patientId, labId)
        for key in item_score.keys():
            correct = float(sum(item_score[key][0:2]))
            #multiple = float(sum(item_score[key][0:3]))
            total = float(sum(item_score[key]))
            print(item_score[key])
            print('%s is %f' % (key, correct/total))
    ...
    #
    def _compare_values(self, validation_record, gold_standard_record, data_item, score, patientId, labId):
        try:
        ...
        if validation_value is not None and gold_standard_value is not None:
            if validation_value != 'MULTIPLE VALUES':
                ...
                score[data_item][2] += 1
                self.logger.log_entry(patientId, labId, data_item, gold_standard_value, validation_value)
        elif validation_value is not None and gold_standard_value is None:
            score[data_item][4] += 1
            self.logger.log_entry(patientId, labId, data_item, gold_standard_value, validation_value)
        elif validation_value is None and gold_standard_value is not None:
            score[data_item][5] += 1
            self.logger.log_entry(patientId, labId, data_item, gold_standard_value, validation_value)
        elif validation_value is None and gold_standard_value is None:
            score[data_item][1] += 1
        return score
    ...
    def _get_data(self):
        ...
        validation_object = BeatAML_Waves_1_And_2_packager(self.data_dir, self.directory_manager)
