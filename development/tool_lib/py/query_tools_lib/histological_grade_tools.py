...
class Summarization(Preprocessor_base):
    ...
    def _process_mitotic_rate(self):
        self.text = \
            self.lambda_manager.lambda_conversion('(?i)(points )?for mito(s(e|i)s|tic)( (activity|count|figure(s)?|index|rate))', self.text, 'for mitoses')
        ...
        self.text = \
            self.lambda_manager.contextual_lambda_conversion(match_str0, match_str1, self.text, 'mitoses = ')
        ...
        self.text = \
            self.lambda_manager.contextual_lambda_conversion(match_str0, match_str1, self.text, 'mitoses = ')
        self.text = \
            self.lambda_manager.lambda_conversion('(?i)mito(ses|sis|tic) (activity|count|figure(s)?|index|rate)', self.text, 'mitoses')
        self.text = \
            self.lambda_manager.lambda_conversion('(?i)mitos(e|i)s', self.text, 'mitoses')
        self.text = \
            self.lambda_manager.lambda_conversion('(?i)mito(ses|sis|tic) (activity|count|figure(s)?|index|rate)', self.text, 'mitoses')
        self.text = \
            self.lambda_manager.lambda_conversion('(?i) mitoses per ', self.text, '/')
        self.text = \
            self.lambda_manager.lambda_conversion(' a mitoses', self.text, ' mitoses')
        
    #
    def _process_nuclear_pleomorphism(self):
        self.text = \
            self.lambda_manager.lambda_conversion('(?i)pleomorphism', self.text, 'nuclear pleomorphism')
        self.text = \
            self.lambda_manager.lambda_conversion('(?i)nuclear nuclear', self.text, 'nuclear')
        self.text = \
            self.lambda_manager.lambda_conversion('(?i)(points )?for nuclear (atypia|grade|pleomorphism|score)', self.text, 'for nuclei')
        match_str = '(?i)nuclear (atypia|grade|pleomorphism|score) \( \d \)'
        self.text = \
            self.lambda_manager.contextual_lambda_conversion(match_str, ' \)', self.text, '')
        match_str = '(?i)nuclear (atypia|grade|pleomorphism|score) \( \d'
        self.text = \
            self.lambda_manager.contextual_lambda_conversion(match_str, '\( ', self.text, '')
        ...
        self.text = \
            self.lambda_manager.contextual_lambda_conversion(match_str0, match_str1, self.text, 'nuclei = ')
        ...
        self.text = \
            self.lambda_manager.contextual_lambda_conversion(match_str0, match_str1, self.text, 'nuclei = ')
        self.text = \
            self.lambda_manager.lambda_conversion('(?i)nuclei', self.text, 'nuclei')
        self.text = \
            self.lambda_manager.lambda_conversion('(?i)nuclear (atypia|grade|pleomorphism|score)', self.text, 'nuclei')
        self.text = \
            self.lambda_manager.lambda_conversion(' a nuclei', self.text, ' nuclei')
    ...
    def _process_tubule_formation(self):
        ...
        self.text = \
            self.lambda_manager.lambda_conversion(match_str, self.text, 'for tubules')
        ...
        match_str1 = '(?i)(glandular )?(\(acinar\)-)?tub(al|ular|ule(s)?)' + \
                     '( (differentiation|formation))?'
        self.text = \
            self.lambda_manager.contextual_lambda_conversion(match_str0, match_str1, self.text, 'tubules = ')
        match_str0 = '(?i)(glandular )?(\(acinar\)-)?tub(al|ular|ule(s)?)' + \
                     '( (differentiation|formation))?' + \
                     '(:)?((\s)?(grade|score)?( of)?)? \d'
        match_str1 = '(?i)(glandular )?(\(acinar\)-)?tub(al|ular|ule(s)?)' + \
        ...
        match_str = '(?i)(glandular )?(\(acinar\)-)?tub(al|ular|ule(s)?)' + \
                    '( (differentiation|formation))'
        self.text = \
            self.lambda_manager.lambda_conversion(match_str, self.text, 'tubules')
        self.text = \
            self.lambda_manager.lambda_conversion(' a tubules', self.text, ' tubules')
