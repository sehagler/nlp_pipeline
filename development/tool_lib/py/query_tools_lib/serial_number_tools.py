...
class Summarization(Preprocessor_base):
    ...
    def _remove_part(self):
        self.text = \
            self.lambda_manager.deletion_lambda_conversion('(?i)\s*\( part' + s() + '-' + part_label() + '((-to-|-)' + part_label() + ' )? \)', self.text)
        #match_str = '(?i)((\n\s*)?-( )?)?(please[\n\s]+)?see (also )?' + \
    ...
    def _remove_slice(self):
        ...
        self.text = \
            self.lambda_manager.deletion_lambda_conversion('(?i)slice (#( )?)?' + slice_label() + '((,)? )?(with clip((,)? )?)?', self.text)
        
    #
    def _remove_slide(self):
        self.text = \
            self.lambda_manager.deletion_lambda_conversion('(?i)\( slide' + s() + ' [^n\)\]]+ \)', self.text)
        self.text = \
        ...
        self.text = \
            self.lambda_manager.deletion_lambda_conversion('(?i)(, )?slide ' + slide_label(), self.text)
        
    #
    def _remove_test(self):
        self.text = \
            self.lambda_manager.deletion_lambda_conversion('(?i)\( test # ' + test_label() + ' \)', self.text)
