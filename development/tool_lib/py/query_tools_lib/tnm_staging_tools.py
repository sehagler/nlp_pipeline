...
class Postprocessor(Postprocessor_base):
    ...
    def _cleanup_data_table(self, data_table):
        for i in range(len(data_table)):
            data_table[i][1] = \
                self.lambda_manager.lambda_conversion(',', data_table[i][1], '')
            data_table[i][1] = \
                self.lambda_manager.lambda_conversion(' +', data_table[i][1], '')
        return data_table
    ...
    def _normalize_staging(self, staging):
        if staging is not None:
            staging = self.lambda_manager.lambda_conversion('[Oo]', staging, '0')
            staging = self.lambda_manager.lambda_conversion('x', staging, 'X')
            staging = self.lambda_manager.lambda_conversion('n/a', staging, '(n/a)')
            staging = self.lambda_manager.lambda_conversion('\(\(', staging, '(')
            staging = self.lambda_manager.lambda_conversion('\)\)', staging, ')')
        return staging
