...
#
def lambda_conversion(abstraction_operator, expression, argument):
    return re.sub('(?i)' + abstraction_operator, argument, expression)

#
def contextual_lambda_conversion(context_abstraction_operator,
    ...
    while not stop_flg:
        ...
        for match in re.finditer('(?i)' + context_abstraction_operator, expression):
            if match is not None:
                ...
                match_argument = \
                    lambda_conversion(abstraction_operator,
                                      search_str, argument)
...
#
def deletion_lambda_conversion(abstraction_operator, expression):
    return lambda_conversion(abstraction_operator, expression, '')

#
def erasure_lambda_conversion(abstraction_operator, expression):
    return lambda_conversion(abstraction_operator, expression, ' ')

#
def initialism_lambda_conversion(abstraction_operator, expression, argument):
    expression = lambda_conversion(abstraction_operator + '( \( ?' + argument + ' ?\))?',
                                   expression, argument)
    return lambda_conversion(argument + ' \( ?' + argument + ' ?\)',
                             expression, argument)

    #
def space_correction_lambda_conversion(abstraction_operator, expression, argument):
    return lambda_conversion(abstraction_operator, expression, ' ' + argument + ' ')
